...
const SYMBOL_PALETTE = [
  'circle-open', 'square-open', 'triangle-up-open', 'diamond-open',
  'cross-open', 'x-thin-open', 'triangle-down-open', 'star-open',
  'pentagon-open', 'hexagon-open', 'triangle-right-open', 'triangle-left-open'
];

const COLOR_PALETTE = [
  '#e41a1c', '#ff7f00', '#ffd700', '#7fff00',
  '#32cd32', '#00cc99', '#00cfe3', '#1e90ff',
  '#6a5acd', '#ba55d3', '#ff4da6', '#ff5a5f'
];

const getExtendedGroupColor = (index: number): string => {
...
const WorkbenchDetailPCAScatterPlot: React.FC<WorkbenchDetailPCAScatterPlotProps> = ({ workbenchId }) => {
  ...
  const plotData = useMemo(() => {
    ...
    orderedGroups.forEach((group, index) => {
      // Replicates in the same group share a single marker style.
      groupColors[group] = getExtendedGroupColor(index);
      groupSymbols[group] = SYMBOL_PALETTE[index % SYMBOL_PALETTE.length];
    });
