...
interface MfuzzClusterGeneTableProps {
  genes: MfuzzGene[];
  samples: string[];
  clusterId: number;
  workbenchId: number;
...
const MfuzzClusterGeneTable: React.FC<MfuzzClusterGeneTableProps> = ({
  genes: initialGenes,
  ...
  // 백엔드에서 가져온 데이터 상태
  const [clusterData, setClusterData] = useState<MfuzzClusterDataResponse | null>(null);
  ...
  const {
    ...
    description: `Mfuzz Cluster ${clusterId} genes`,
  });
  const [selectAllState, setSelectAllState] = useState<0 | 1 | 2>(0);

  // Fetch cluster data from backend
  const fetchClusterData = useCallback(async (page: number, search: string, limit: number) => {
    setLoading(true);
    try {
      ...
      console.error('❌ [MFUZZ] Failed to fetch cluster data:', error);
    } finally {
      setLoading(false);
    }
  }, [workbenchId, clusterId, params]);
  ...
  const fetchAllGeneIds = useCallback(async (): Promise<string[]> => {
    try {
      ...
      return (response.genes || []).map((g: MfuzzGene) => g.gene_id);
    } catch (error) {
      console.error('Failed to fetch all gene IDs:', error);
      return [];
    }
  }, [workbenchId, clusterId, params, activeSearch, totalGenes]);
  ...
  const tableHeaderRow = (
    <tr>
      ...
      <th className="px-2 py-2 text-left text-xs font-bold text-slate-600 uppercase tracking-wider" style={{ width: `${tableColumnWidths.membership}px`, minWidth: `${tableColumnWidths.membership}px`, backgroundColor: 'rgb(248 250 252)' }}>Membership</th>
      {samples.map(sample => (
  ...
  const handleDownload = useCallback(() => {
    ...
    // TSV 형식으로 데이터 생성
    const headers = ['GeneID', 'GeneSymbol', 'Membership', ...samples];
    ...
    const filename = `mfuzz_cluster_${clusterId}_${selectionType}_${timestamp}.tsv`;
  ...
  }, [genes, selectedGenes, clusterId, samples]);

  return (
    <div ref={tableSectionRef} className="mt-6 space-y-4 relative">
      {/* Table Title */}
      <div className="border-b border-slate-200 pb-3">
        <h3 className="text-lg font-semibold text-slate-900">Genes in Cluster {clusterId}</h3>
        <p className="text-sm text-slate-500 mt-1">
          {totalGenes.toLocaleString()} genes · Fuzzy c-means clustering
        </p>
      </div>
      ...
      {/* Search Results Info */}
      {clusterData?.is_search_result && (
        <div className="p-3 bg-blue-50 border border-blue-200 rounded-lg">
          <div className="flex items-center justify-between">
            <span className="text-sm text-blue-800">
              🔍 Search results for "{clusterData.search_query}": {totalGenes.toLocaleString()} gene{totalGenes !== 1 ? 's' : ''} found
            </span>
            <button
              onClick={handleClearSearch}
      ...
      <div className="border border-slate-200 rounded-lg overflow-visible bg-white">
        ...
        <table ref={tableRef} className="data-table" style={{ position: 'relative', width: '100%', minWidth: '800px', tableLayout: 'fixed' }}>
          {tableColGroup}
            ...
            <div className="absolute top-0 overflow-hidden border-r border-b border-slate-200 bg-slate-50 shadow-sm" style={{ left: `${Math.min(fixedHeaderWidth, overlayHeader.width)}px`, width: `${Math.max(0, overlayHeader.width - Math.min(fixedHeaderWidth, overlayHeader.width))}px` }}>
              <div style={{ width: `${Math.max(0, overlayHeader.tableWidth - fixedHeaderWidth)}px`, transform: `translateX(-${overlayHeader.scrollLeft}px)` }}>
                <table className="data-table" style={{ width: `${Math.max(0, overlayHeader.tableWidth - fixedHeaderWidth)}px`, tableLayout: 'fixed' }}>
                  ...
                  <thead>
                    <tr>
                      <th className="px-2 py-2 text-left text-xs font-bold text-slate-600 uppercase tracking-wider" style={{ width: `${tableColumnWidths.membership}px`, minWidth: `${tableColumnWidths.membership}px` }}>Membership</th>
                      {samples.map(sample => (
        ...
        <div className="text-center py-12">
          ...
          <p className="text-slate-500">
            {activeSearch
              ...
              : 'No genes available in this cluster'}
          </p>
        </div>
      )}
      ...
      <GOAnalysisModal
        ...
        comparisonName={`Mfuzz Cluster ${clusterId}`}
        toolName="Mfuzz"
      />
...
export default MfuzzClusterGeneTable;
