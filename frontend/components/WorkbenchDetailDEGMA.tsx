...
interface WorkbenchDetailDEGMAProps {
    workbenchId: number;
...
type MADotSizePreset = 'tiny' | 'small' | 'medium' | 'large' | 'xlarge';
type MAColorTarget = 'significant' | 'background';

const MA_DOT_SIZE_OPTIONS: Array<{
    value: MADotSizePreset;
    label: string;
    significantRadius: number;
    backgroundRadius: number;
}> = [
    { value: 'tiny', label: 'Tiny', significantRadius: 3, backgroundRadius: 1.5 },
    { value: 'small', label: 'Small', significantRadius: 4, backgroundRadius: 2 },
    { value: 'medium', label: 'Medium', significantRadius: 5, backgroundRadius: 3 },
    { value: 'large', label: 'Large', significantRadius: 6.5, backgroundRadius: 4 },
    { value: 'xlarge', label: 'XL', significantRadius: 8, backgroundRadius: 5 }
];
...
const WorkbenchDetailDEGMA: React.FC<WorkbenchDetailDEGMAProps> = ({
    workbenchId,
    comparisonName,
    toolName,
    selectedGenes,
    ...
    // MA Plot data states
    const [plotData, setPlotData] = useState<MAPlotDataPoint[]>([]);
    ...
    const [dotSizePreset, setDotSizePreset] = useState<MADotSizePreset>('medium');
    const [significantDotColor, setSignificantDotColor] = useState<string>('#dc2626');
    const [backgroundDotColor, setBackgroundDotColor] = useState<string>('#94a3b8');
    const [activeColorTarget, setActiveColorTarget] = useState<MAColorTarget>('significant');
    ...
    // Fetch MA Plot data for chart
    const fetchMAPlotData = useCallback(async () => {
        try {
            ...
            console.error('Failed to fetch MA plot data:', err);
            setPlotError('Failed to load MA plot data');
            setPlotData([]);
            setIsPlotLoading(false);
            setIsPlotRendering(false);
        }
    ...
    const upregulatedData = significantData.filter(d => d.logFC > 0);
    const downregulatedData = significantData.filter(d => d.logFC < 0);
    const dotSizeConfig = useMemo(
        () => MA_DOT_SIZE_OPTIONS.find((option) => option.value === dotSizePreset) ?? MA_DOT_SIZE_OPTIONS[2],
        [dotSizePreset]
    );

    // MA Plot chart data: Show only top N significant genes (by FDR)
    ...
    // Chart data with limited significant genes
    const chartPlotData = useMemo(() => {
        const chartSignificantSet = new Set(chartSignificantData.map(d => d.gene_id));

        return plotData
            .filter(d => !d.significant || chartSignificantSet.has(d.gene_id))
            .sort((a, b) => (a.significant === b.significant ? 0 : a.significant ? 1 : -1))
            .map(d => ({
                ...d,
                size: d.significant ? dotSizeConfig.significantRadius : dotSizeConfig.backgroundRadius,
                fill: d.significant ? significantDotColor : backgroundDotColor,
                fillOpacity: d.significant ? 0.95 : 0.35
            }));
    }, [plotData, chartSignificantData, dotSizeConfig, significantDotColor, backgroundDotColor]);

    const applySelectedColor = (color: string) => {
        if (activeColorTarget === 'significant') {
    ...
    const handleExportFigure = useCallback(async (options: ChartExportOptions) => {
        ...
        try {
            ...
            console.error('Failed to export MA plot figure:', error);
            alert('Failed to export MA plot figure. Please try again.');
        } finally {
    ...
    const getRegulationStatus = (d: { logFC: number }) => {
        if (d.logFC > 0) return { text: 'Upregulated', color: 'text-red-600' };
        if (d.logFC < 0) return { text: 'Downregulated', color: 'text-blue-600' };
        return { text: 'No Change', color: 'text-slate-600' };
    };

    // Custom tooltip for MA plot
                ...
                <div className="bg-white p-3 border border-slate-200 rounded-lg shadow-lg">
                    <div className="mb-2">
                        ...
                        {data.gene_symbol && data.gene_symbol !== data.gene_id && (
                            <p className="text-sm text-slate-500 italic">{data.gene_symbol}</p>
                        )}
                    </div>
                    <p className="text-sm text-slate-600">
                        <span className="font-medium">logCPM:</span> {data.logCPM.toFixed(2)}
                    </p>
                    <p className="text-sm text-slate-600">
                    ...
                    {data.significant ? (
                        <p className="text-xs mt-2 text-green-600">??Significant (FDR &lt; 0.05)</p>
                    ) : (
        ...
        return (
            <div className="h-full flex items-center justify-center">
                <div className="text-center">
                    ...
                    <h3 className="text-lg font-semibold text-slate-900 mb-2">Loading MA Plot</h3>
        ...
        return (
            <div className="h-full flex items-center justify-center">
                <div className="text-center">
                    ...
                    <p className="text-sm text-slate-500">No MA plot data found for {comparisonName}</p>
                </div>
            </div>
        );
    }
        ...
        <div ref={tableSectionRef} className="h-full flex flex-col p-6 relative">
            ...
            <div className="mb-6">
                <div className="flex items-center justify-between mb-2">
                    <h3 className="text-lg font-semibold text-slate-900">MA Plot: {comparisonName.replace(/_/g, ' ')}</h3>
                    ...
                    <div className="flex items-center gap-3 flex-wrap justify-end">
                            ...
                            <div className="relative" ref={dotSizeMenuRef}>
                                ...
                                >
                                    <span className="inline-flex items-center justify-center w-6 h-5 ml-2">
                                        <span
                                            ...
                                            style={{
                                                width: `${dotSizeConfig.significantRadius * 2}px`,
                                                height: `${dotSizeConfig.significantRadius * 2}px`,
                                                backgroundColor: significantDotColor
                                            }}
                                        />
                                    </span>
                                ...
                                {isDotSizeMenuOpen && (
                                    <div className="absolute right-0 top-full mt-2 z-40 min-w-[180px] rounded border border-slate-300 bg-white p-1 shadow-xl">
                                        {MA_DOT_SIZE_OPTIONS.map((option) => (
                                            <button
                                                key={option.value}
                                                type="button"
                                                onClick={() => {
                                            ...
                                            >
                                                <span className="inline-flex items-center justify-center w-7 h-5">
                                                    <span
                                                        className="rounded-full border border-slate-400"
                                                        style={{
                                                            width: `${option.significantRadius * 2}px`,
                                                            height: `${option.significantRadius * 2}px`,
                                                            backgroundColor: significantDotColor
                                                        }}
                                                    />
                                                </span>
                        ...
                        <div className="flex items-center space-x-2">
                            ...
                            <select
                                ...
                                onChange={(e) => setActiveColorTarget(e.target.value as MAColorTarget)}
                        ...
                        <div className="relative" ref={paletteRef}>
                            ...
                            >
                                <span
                                    ...
                                    style={{ backgroundColor: activeColorTarget === 'significant' ? significantDotColor : backgroundDotColor }}
                                />
                                <span>Palette</span>
                            </button>
                            {isColorPaletteOpen && (
                ...
                <p className="text-sm text-slate-600">
                    Average expression (logCPM) vs log fold change (logFC) ??Tool: {toolName} ??
                    Total genes: {plotData.length.toLocaleString()} ??
                    Significant: <span className="font-semibold text-red-600">{significantData.length.toLocaleString()}</span>
                </p>
            </div>
            ...
            <div className="w-full bg-white">
                ...
                >
                    ...
                    <ResponsiveContainer width="100%" height="100%">
                        <ScatterChart
                            key={`ma-plot-${comparisonName}-${plotData.length}-${chartDisplayLimit}-${dotSizePreset}`}
                        ...
                        >
                            ...
                            <XAxis
                                ...
                                dataKey="logCPM"
                                name="logCPM"
                                label={{ value: 'Average Expression (logCPM)', position: 'insideBottom', offset: -10, style: { fontSize: 14, fill: '#475569' } }}
                                stroke="#64748b"
                                tick={{ fontSize: 12 }}
                            />
                            <YAxis
                            ...
                            <Scatter
                                key={`all-data-${plotData.length}-${chartDisplayLimit}-${dotSizePreset}`}
                                name="MA Plot"
                                data={chartPlotData}
                                shape={(props: any) => {
            ...
            <div className="mt-4 p-4 bg-slate-50 rounded-lg border border-slate-200">
                <div className="flex items-start justify-between mb-2">
                    <p className="text-xs text-slate-600 flex-1">
                        <strong>Interpretation:</strong> Genes further from the center (logFC = 0) show stronger differential expression.
                        Genes with higher logCPM values are more reliably detected.
                        Red points indicate statistically significant differences (FDR &lt; 0.05).
            ...
            <ChartExportModal
                ...
                defaultFilename={`${comparisonName.replace(/\s+/g, '_')}_MA_plot`}
...
export default WorkbenchDetailDEGMA;
