...
const parseGeneSearchTerms = (searchQuery: string) =>
    searchQuery
        .split(/[\s,;\t\n]+/)
        .map(term => term.trim().toUpperCase())
        .filter(term => term.length > 0);

const resolveGeneSearchMode = (searchMode: SearchMode, termCount: number): 'exact' | 'contains' => {
    if (searchMode === 'exact') return 'exact';
    if (searchMode === 'contains') return 'contains';
    return termCount > 1 ? 'exact' : 'contains';
};
...
const WorkbenchDetailCounts: React.FC<WorkbenchDetailCountsProps> = ({
    ...
    const [activeSearchMode, setActiveSearchMode] = useState<SearchMode>('auto');
     ...
     */
    const fetchCountResults = async (
        matrixType: MatrixType,
        page: number = 1,
        search: string = '',
        limit: number = 1000,
        mode: SearchMode = activeSearchMode
    ) => {
        ...
        try {
            ...
            if (search) {
                ...
                url += `&search_mode=${encodeURIComponent(mode)}`;
            }
     ...
     */
    const filterPatternResults = (searchQuery: string, mode: SearchMode = activeSearchMode) => {
        if (!patternFullResults || !patternFullResults.matrix) {
            return;
        }

        const searchTerms = parseGeneSearchTerms(searchQuery);
        const resolvedMode = resolveGeneSearchMode(mode, searchTerms.length);
        ...
        const filteredMatrix = patternFullResults.matrix.filter((gene: any) => {
            ...
            if (resolvedMode === 'exact') {
                return searchTermSet.has(geneId) || searchTermSet.has(geneSymbol);
            }

            return searchTerms.some(term => geneId.includes(term) || geneSymbol.includes(term));
        });
    ...
    const handleSearch = () => {
        ...
        if (searchValue) {
            ...
            if (isPatternAnalysisActive && patternFullResults) {
                filterPatternResults(searchValue, searchMode);
            } else {
                // Regular search in full dataset
                fetchCountResults(selectedMatrixType, 1, searchValue, pageSize, searchMode);
            }
        }
    };
    ...
    const handlePageChange = (newPage: number) => {
        ...
        if (!isPatternAnalysisActive) {
            fetchCountResults(selectedMatrixType, safePage, activeSearch, pageSize, activeSearchMode);
        }
    ...
    const handlePageSizeChange = (newSize: number) => {
        ...
        // Pattern analysis pagination is handled on the client side.
        if (!isPatternAnalysisActive) {
            fetchCountResults(selectedMatrixType, 1, activeSearch, newSize, activeSearchMode);
        }
    };
    ...
    useEffect(() => {
        if (progressData?.status === 'completed') {
            fetchCountResults(selectedMatrixType, currentPage, activeSearch, pageSize, activeSearchMode);
    ...
    const renderSampleHeaderLabel = (name: string) => (
        <span className="leading-tight">
            {name.split('_').map((part, index, parts) => (
    ...
    const sampleHeaderStyle = (isSelected: boolean): React.CSSProperties => ({
    ...
    const fetchAllGeneIds = useCallback(async (): Promise<string[]> => {
        if (isPatternAnalysisActive && patternFullResults) {
            ...
            if (activeSearch) {
                ...
                url += `&search_mode=${encodeURIComponent(activeSearchMode)}`;
            }
            try {
                const response = await fetch(url, { credentials: 'include' });
                if (response.ok) {
    ...
    }, [workbench.id, isPatternAnalysisActive, patternFullResults, countsResults?.total_genes, selectedMatrixType, activeSearch, activeSearchMode]);
        ...
        <tr>
            ...
            {(countsResults?.samples || []).map(name => {
                ...
                return (
                    <th
                        key={name}
                        className="px-3 py-2 text-left text-xs font-bold uppercase tracking-wider cursor-pointer hover:bg-blue-50 transition-colors"
                        style={sampleHeaderStyle(isSelected)}
                        onClick={() => handleColumnClick(name)}
                        title={`Click to ${isSelected ? 'deselect' : 'select'} ${name} column`}
                    >
                        {renderSampleHeaderLabel(name)}
                    </th>
                );
            })}
        </tr>
                        ...
                        <div ref={tableSectionRef} className="relative">
                            ...
                            <div className="mb-4 flex items-center justify-between">
                                <div className="flex flex-wrap items-center gap-5">
                                    ...
                                    <label className="sr-only" htmlFor="count-search-mode">Search match mode</label>
                                    <select
                                        id="count-search-mode"
                                        value={searchMode}
                                        onChange={(e) => setSearchMode(e.target.value as SearchMode)}
                                        className="px-3 py-2 border border-slate-300 rounded-lg bg-white text-slate-700 focus:ring-primary focus:border-primary text-sm"
                                        title="Auto: lists use exact matching, single terms use contains matching"
                                    >
                                ...
                                <div className="mb-4 p-3 bg-blue-50 border border-blue-200 rounded-lg">
                                    <div className="flex items-center justify-between">
                                        <span className="text-sm text-blue-800">
                                            ...
                                            {countsResults.search_mode && (
                                                <span className="ml-2 rounded bg-blue-100 px-2 py-0.5 text-xs uppercase text-blue-700">
                                                    {countsResults.search_mode}
                                                </span>
                                            )}
                                        </span>
