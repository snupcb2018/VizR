            ...
            try:
                ...
                if self.api_key:
                    ...
                    if cleaned_api_key != self.api_key:
                        logger.warning("Google AI client - API key had surrounding whitespace and was trimmed")
                        self.api_key = cleaned_api_key
                    logger.info("Google AI client test_connection - API key configured (length: %s)", len(self.api_key))
                else:
                    logger.error("Google AI client test_connection - API key is None or empty")
