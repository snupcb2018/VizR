...
def safe_join_interesting_genes_path(base_dir: str, relative_path: str) -> str:
    """Return a path under base_dir, rejecting absolute paths and traversal."""
    ...
    if not normalized:
        raise ValueError("Path is required")
    if '\x00' in normalized or normalized.startswith('/') or os.path.isabs(normalized):
    ...
    candidate = base_path.joinpath(*parts).resolve() if parts else base_path
    if candidate != base_path and base_path not in candidate.parents:
        raise ValueError("Path must stay inside the interesting genes directory")
...
def get_genes():
    ...
    try:
        ...
        # 전체 파일 경로 생성
        full_path = safe_join_interesting_genes_path(interesting_genes_dir, file_path)
...
def create_gene_set():
    ...
    try:
        ...
        full_folder_path = safe_join_interesting_genes_path(interesting_genes_dir, folder_path)
        ...
        full_file_path = safe_join_interesting_genes_path(full_folder_path, safe_file_name)
        ...
        relative_path = os.path.join(folder_path, safe_file_name).replace('\\', '/')
...
def delete_folder():
    ...
    try:
        ...
        # 전체 폴더 경로 생성
        full_path = safe_join_interesting_genes_path(interesting_genes_dir, folder_path)
...
def upload_gene_sets():
    ...
    try:
        ...
        # 대상 폴더 경로 생성
        base_target_folder = safe_join_interesting_genes_path(interesting_genes_dir, folder_path)
        ...
        for file in files:
            ...
            if any(part in ('', '.', '..') for part in path_parts):
                logger.warning(f"⚠️ [INTERESTING-GENES] Skipping unsafe file path: {file.filename}")
                continue

            safe_path_parts = [secure_filename(part) for part in path_parts]
            if any(not part for part in safe_path_parts):
                logger.warning(f"⚠️ [INTERESTING-GENES] Skipping unsafe file path: {file.filename}")
                continue
            safe_relative_path = '/'.join(safe_path_parts)

            # 최종 파일 경로
            file_path = safe_join_interesting_genes_path(base_target_folder, safe_relative_path)
