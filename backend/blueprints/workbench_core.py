...
def get_import_workbench_sources():
    ...
    try:
        ...
        with get_db_connection() as conn:
            workbenches = conn.execute('''
                SELECT w.id, w.name, w.species, w.user_id, u.username
            ...
            for workbench in workbenches:
                ...
                if not count_tool or count_tool == 'imported_counts':
                    continue
    ...
    try:
        ...
        if 'clean' in step_details:
            ...
            if 'trimmomatic' in clean_tools:
                adapter = params.get('trimmomaticAdapter', 'TruSeq3')
                seed_mm = params.get('illuminaclipSeedMismatches', 2)
                palindrome = params.get('illuminaclipPalindromeClip', 30)
                simple = params.get('illuminaclipSimpleClip', 10)
                ...
                # Build adapter file name
                adapter_file = adapter
                if layout == 'pe' and not adapter.endswith('.fa'):
                    adapter_file = f"{adapter}-PE-2.fa"
                elif layout == 'se' and not adapter.endswith('.fa'):
                    adapter_file = f"{adapter}-SE.fa"
