...
def parse_count_search_terms(search_query: str):
    """Split count search text pasted from single-line or multi-line sources."""
    return [term.strip().upper() for term in re.split(r'[,;\s\t\n]+', search_query) if term.strip()]


def normalize_count_search_mode(search_mode: str):
    mode = str(search_mode or 'auto').strip().lower()
    if mode in {'exact', 'exact-list'}:
        return 'exact'
    if mode in {'contains', 'substring', 'like'}:
        return 'contains'
    return 'auto'


def filter_counts_by_gene_search(
    df,
    gene_id_col,
    search_query,
    gene_annotations=None,
    log_prefix='COUNTS-SEARCH',
    search_mode='auto'
):
    """
    Count table gene search.

    search_mode:
      - auto: one term uses contains; pasted multi-term lists use exact.
      - exact: gene_id/gene_symbol must match a term exactly.
      - contains: gene_id/gene_symbol may contain a term, like the old search.
    """
    search_terms = parse_count_search_terms(search_query)
    logger.info(f"🔍 [{log_prefix}] Parsed {len(search_terms)} search terms")

    if not search_terms:
        logger.info(f"ℹ️ [{log_prefix}] Empty search terms after parsing")
        return df, {
            'terms': search_terms,
            'mode': 'empty',
            'requested_mode': normalize_count_search_mode(search_mode),
            'matched_by_id': 0,
            'matched_by_symbol': 0,
    ...
    requested_mode = normalize_count_search_mode(search_mode)
    exact_list_mode = requested_mode == 'exact' or (requested_mode == 'auto' and len(search_terms) > 1)
    mode = 'exact' if exact_list_mode else 'contains'
    gene_ids = df[gene_id_col].astype(str)
    gene_ids_upper = gene_ids.str.upper()
    gene_symbols = gene_ids.map(lambda gene_id: gene_annotations.get(gene_id, {}).get('symbol', ''))
    gene_symbols_upper = gene_symbols.astype(str).str.upper()

    if exact_list_mode:
        search_term_set = set(search_terms)
        id_mask = gene_ids_upper.isin(search_term_set)
        symbol_mask = gene_symbols_upper.isin(search_term_set)
    else:
        id_mask = pd.Series([False] * len(df), index=df.index)
        symbol_mask = pd.Series([False] * len(df), index=df.index)
        for term in search_terms:
            id_mask |= gene_ids_upper.str.contains(term, regex=False, na=False)
            symbol_mask |= gene_symbols_upper.str.contains(term, regex=False, na=False)

    mask = id_mask | symbol_mask
    filtered_df = df[mask]
    matched_by_id = int(id_mask.sum())
    matched_by_symbol = int(symbol_mask.sum())

    logger.info(f"✅ [{log_prefix}] Search complete: {len(filtered_df)} genes matched (mode={mode})")
    logger.info(f"   ├─ By gene_id: {matched_by_id}")
    logger.info(f"   └─ By gene_symbol: {matched_by_symbol}")

    if len(filtered_df) == 0:
        logger.warning(f"⚠️ [{log_prefix}] No genes matched search query: {search_terms[:10]}")
...
def get_counts_result(workbench_id):
    ...
    search_mode = normalize_count_search_mode(request.args.get('search_mode', 'auto'))
    ...
    logger.info(f"📊 [COUNTS-RESULTS-API] Request params: matrix_type={matrix_type}, page={page}, limit={limit}, offset={offset}, search_mode={search_mode}, search={search_query[:100] if search_query else 'None'}")
    ...
    try:
        ...
        with get_db_connection() as conn:
            ...
            if search_query:
                ...
                df, search_meta = filter_counts_by_gene_search(
                    df,
                    gene_id_col,
                    search_query,
                    log_prefix='COUNTS-RESULTS-API',
                    search_mode=search_mode
                )
                total_genes = len(df)
                logger.info(f"🔍 [COUNTS-RESULTS-API] Search mode: {search_meta['mode']}")
...
def get_selected_counts_result(workbench_id):
    ...
    try:
        ...
        search_mode = normalize_count_search_mode(request_data.get('search_mode', 'auto'))
        ...
        logger.info(
            ...
            f"search_mode={search_mode}, search={search_query[:100] if search_query else 'None'}"
        )
        ...
        with get_db_connection() as conn:
            ...
            if search_query:
                df, search_meta = filter_counts_by_gene_search(
                    ...
                    log_prefix='COUNTS-SELECTED-API',
                    search_mode=search_mode
                )
                logger.info(f"[COUNTS-SELECTED-API] Search filter complete: matched={len(df)}, mode={search_meta['mode']}")
...
def download_counts_result(workbench_id):
    ...
    if request.method == 'POST':
        ...
        if body.get('search_mode') is not None:
            search_mode = normalize_count_search_mode(body.get('search_mode', 'auto'))
        ...
        with get_db_connection() as conn:
            ...
            if search_query:
                ...
                df, search_meta = filter_counts_by_gene_search(
                    ...
                    gene_id_col,
                    search_query,
                    log_prefix='DOWNLOAD-API',
                    search_mode=search_mode
                )
                logger.info(f"🔍 [DOWNLOAD-API] Search mode: {search_meta['mode']}")
