...
R 스크립트를 사용한 DESeq2 기반 차등발현분석을 수행합니다.
"""
...
def cleanup_deseq2_temp_files(*file_paths):
    """DESeq2 임시 파일들 정리"""
        ...
        if file_path and os.path.exists(file_path):
            try:
                if os.path.isdir(file_path):
                    ...
                    logger.info(f"🗑️ [DESEQ2-CLEANUP] Removed temp directory: {os.path.basename(file_path)}")
                else:
                    os.remove(file_path)
                    logger.info(f"🗑️ [DESEQ2-CLEANUP] Removed temp file: {os.path.basename(file_path)}")
            except Exception as e:
                logger.warning(f"⚠️ [DESEQ2-CLEANUP] Failed to remove {file_path}: {e}")
...
def execute_deseq2(job_step: sqlite3.Row, worker_id: str = "worker") -> Dict[str, str]:
    """DESeq2를 이용한 차등발현분석 (Trinity run_DE_analysis.pl 사용)
    ...
    try:
        ...
        logger.info(f"🧬 [DESEQ2] Starting DESeq2 differential expression analysis")
        ...
        logger.info(f"📊 [DESEQ2] Analysis parameters: {parameters}")
        ...
        deseq2_dir = os.path.join(base_dir, "DESeq2")
        os.makedirs(deseq2_dir, exist_ok=True)
        
        logger.info(f"📁 [DESEQ2] Directory setup:")
        logger.info(f"   ├─ Base directory: {base_dir}")
        logger.info(f"   └─ DESeq2 output: {deseq2_dir}")
        
        # ✅ DESeq2 임시 파일들 정리를 위한 cleanup 등록
        temp_files = [
            os.path.join(deseq2_dir, "tmp"),
            os.path.join(deseq2_dir, "Rplots.pdf"),
            os.path.join(deseq2_dir, ".Rhistory"),
            os.path.join(base_dir, "Rplots.pdf")
        ]
        
        wrapper.register_cleanup(cleanup_deseq2_temp_files, *temp_files)
        logger.info(f"🗑️ [DESEQ2] Registered cleanup for {len(temp_files)} temp files/directories")
        ...
        logger.info(f"📋 [DESEQ2] Input files:")
        ...
        logger.info(f"📊 [DESEQ2] Replicate analysis:")
        ...
        # dispersion 파라미터 결정 (DESeq2도 replicate 부족 시 필요)
        dispersion_param = ""
        if min_replicates < 2:
            if min_replicates == 1:
            ...
            logger.info(f"⚠️  [DESEQ2] Insufficient replicates detected, adding dispersion parameter: {dispersion_value}")
        else:
            logger.info(f"✅ [DESEQ2] Sufficient replicates found, no dispersion parameter needed")
        
        # 6. Docker 명령어 구성 - run_DE_analysis.pl 사용 (상대 경로)
        logger.info(f"🐳 [DESEQ2] Preparing Trinity run_DE_analysis.pl command...")
        ...
        logger.info(f"🐳 [DESEQ2] Docker path mapping:")
        logger.info(f"   ├─ Container path: {workbench_schema['base']}")
        logger.info(f"   ├─ Host VIZR_PATH: {SharedConfig.HOST_VIZR_PATH}")
        logger.info(f"   └─ Host workbench path: {host_workbench_root}")

        # Docker 명령어 구성 (상대 경로 사용)
        docker_cmd = [
            "docker", "run", "--rm",
            ...
            f"--output deg/DESeq2{dispersion_param}"
        ]
        
        logger.info(f"🚀 [DESEQ2] Running Trinity DESeq2 analysis...")
        logger.info(f"   └─ Command: {' '.join(docker_cmd)}")
        
        # ✅ ProcessWrapper로 Trinity Docker 실행
        logger.info(f"🔧 [DESEQ2] Starting ProcessWrapper execution...")
        ...
        if result['stopped_by_user']:
            logger.info(f"🛑 [DESEQ2] DESeq2 analysis stopped by user signal")
            return {
                'success': False,
                'error': 'DESeq2 analysis stopped by user signal',
                'method': 'DESeq2',
        ...
        execution_time = time.time() - start_time
        logger.info(f"⏱️  [DESEQ2] Trinity execution completed in {execution_time:.2f} seconds")
        logger.info(f"   ├─ Success: {result.get('success', False)}")
        logger.info(f"   └─ Execution time: {execution_time:.2f}s")
        
        # 7. 실행 결과 확인
        if not result.get('success', False):
            logger.error(f"❌ [DESEQ2] Trinity DESeq2 analysis failed!")
            ...
            return {
                'success': False,
                'error': f"Trinity DESeq2 failed: {result.get('stderr', 'Unknown error')}",
        ...
        expected_output_files = [
            "DESeq2.DE_results",
            "DESeq2.DE_results.MA_n_Volcano_plots.pdf",
            "DESeq2.count_matrix.RData"
        ]
        ...
        for filename in expected_output_files:
            output_path = os.path.join(deseq2_dir, filename)
            if os.path.exists(output_path):
                output_files.append(output_path)
                file_size = os.path.getsize(output_path)
                logger.info(f"✅ [DESEQ2] Generated: {filename} ({file_size:,} bytes)")
            else:
                missing_files.append(filename)
                logger.warning(f"⚠️ [DESEQ2] Missing: {filename}")
        
        # 9. 결과 요약
        logger.info(f"")
        logger.info(f"📊 [DESEQ2] ╔══════════════════════════════════════════════════════════════════╗")
        logger.info(f"📊 [DESEQ2] ║                   DESEQ2 ANALYSIS SUMMARY                       ║")
        logger.info(f"📊 [DESEQ2] ╚══════════════════════════════════════════════════════════════════╝")
        logger.info(f"✅ [DESEQ2] DESeq2 analysis completed successfully!")
        logger.info(f"   ├─ Generated files: {len(output_files)}")
        logger.info(f"   ├─ Missing files: {len(missing_files)}")
        logger.info(f"   ├─ Output directory: {deseq2_dir}")
        logger.info(f"   ├─ Counts matrix: {counts_matrix_file}")
        logger.info(f"   ├─ Samples file: {samples_file}")
        logger.info(f"   └─ Execution time: {execution_time:.2f}s")
        ...
        execution_time = time.time() - start_time
        error_msg = str(e)
        logger.error(f"❌ [DESEQ2] DESeq2 analysis failed!")
        logger.error(f"   ├─ Error: {error_msg}")
...
def REGISTER_TOOL(coordinator):
    """🔌 자동 tool 등록 함수 - 코디네이터에 deseq2 tool 등록"""
    logger.info(f"🔌 [TOOL-REG] Registering deseq2 tool...")
    
    metadata = {
        'name': 'deseq2',
        'description': 'DESeq2 differential expression analysis',
        'version': '1.0.0',
        'supported_formats': ['.txt', '.csv', '.tsv'],
        'requirements': ['R', 'DESeq2', 'BiocManager'],
        'parameters': {
            'padj_threshold': 0.05,
            'lfc_threshold': 1.0,
    ...
    return coordinator.register_tool('deseq2', execute_deseq2, metadata)
