...
def run_single_trimmomatic(workbench_id, sample_info: Dict[str, Any], parameters: Dict, worker_id: str = "worker", stop_event=None, shared_db_lock=None) -> Dict[str, Any]:
    ...
    try:
        ...
        def _get_param(*names, default=None):
            for name in names:
                if name in parameters and parameters[name] not in (None, ''):
                    return parameters[name]
            return default

        def _get_workbench_username():
            try:
                with database.get_db_connection() as conn:
                    row = conn.execute(
                        """
                ...
                logger.warning(f"⚠️ [TRIMMOMATIC-{os.getpid()}] Failed to resolve workbench username: {e}")
            return None

        def _failure(message):
            logger.error(f"❌ [TRIMMOMATIC-{os.getpid()}] {message}")
            return {
                'success': False,
                'sample_name': sample_name,
                'condition': condition,
        ...
        slidingwindow = _get_param('trimmomatic_slidingwindow', 'slidingwindow', default=None)
        if slidingwindow is not None:
            slidingwindow_parts = str(slidingwindow).split(':', 1)
            if len(slidingwindow_parts) == 2:
                slidingwindow_size = _as_int(slidingwindow_parts[0], 4)
                slidingwindow_quality = _as_int(slidingwindow_parts[1], 15)
            else:
                logger.warning(f"⚠️ [TRIMMOMATIC-{os.getpid()}] Invalid SLIDINGWINDOW value '{slidingwindow}', using 4:15")
                slidingwindow_size = 4
                slidingwindow_quality = 15
        else:
            slidingwindow_size = _as_int(_get_param('trimmomatic_slidingwindow_size', default=4), 4)
            slidingwindow_quality = _as_int(_get_param('trimmomatic_slidingwindow_quality', default=15), 15)

        # Trimmomatic 파라미터 처리 - pipeline step parameters가 단일 source of truth
        trimmomatic_params = {
            'trimmomatic_threads': _as_int(_get_param('trimmomatic_threads', 'threads', default=4), 4),
            'trimmomatic_leading': _as_int(_get_param('trimmomatic_leading', 'leading', default=3), 3),
            'trimmomatic_trailing': _as_int(_get_param('trimmomatic_trailing', 'trailing', default=3), 3),
            'trimmomatic_slidingwindow_size': slidingwindow_size,
            'trimmomatic_slidingwindow_quality': slidingwindow_quality,
            'trimmomatic_minlen': _as_int(_get_param('trimmomatic_minlen', 'minlen', default=36), 36),
            'illuminaclip_seed_mismatches': _as_int(_get_param('illuminaclipSeedMismatches', 'illuminaclip_seed_mismatches', default=2), 2),
            'illuminaclip_palindrome_clip': _as_int(_get_param('illuminaclipPalindromeClip', 'illuminaclip_palindrome_clip', default=30), 30),
            'illuminaclip_simple_clip': _as_int(_get_param('illuminaclipSimpleClip', 'illuminaclip_simple_clip', default=10), 10)
        }

        # 어댑터 파일명 결정 (UI/pipeline parameters에서 버전명 가져와서 파일명으로 변환)
        adapter_version = str(_get_param('trimmomaticAdapter', 'trimmomatic_adapter', 'adapter', default='TruSeq3'))

        # 버전명과 파일 타입(SE/PE)을 조합하여 실제 어댑터 파일명 생성
        if adapter_version.lower() == 'none':
            ...
            if adapter_version in adapter_map:
                if file_type not in adapter_map[adapter_version]:
                    return _failure(f"Adapter '{adapter_version}' is only supported for paired-end data, but sample '{sample_name}' is single-end")
        ...
        if adapter_filename:
            ...
            workbench_username = _get_workbench_username()
            adapter_base_candidates = []

            if workbench_username:
                adapter_base_candidates.append(
                    os.path.join(
            ...
            adapter_base_candidates.extend([
                os.path.join(SharedConfig.VIZR_PATH, "tools", "trimmomatic", "Trimmomatic-0.36", "adapters"),
                os.path.join(SharedConfig.VIZR_PATH, "resource", "tools", "trimmomatic", "Trimmomatic-0.36", "adapters"),
                "/vizr/resource/tools/trimmomatic/Trimmomatic-0.36/adapters"
            ])

            adapter_full_path = None
            for candidate_dir in adapter_base_candidates:
                candidate_path = os.path.join(candidate_dir, adapter_filename)
                if os.path.exists(candidate_path):
                    adapter_full_path = candidate_path
                    break
            ...
            if adapter_full_path is None and not os.path.exists(adapter_base_dir):
                source_adapter_dir = next((path for path in adapter_base_candidates if os.path.exists(path)), None)
                if source_adapter_dir:
                    ...
                    logger.warning(f"⚠️ [TRIMMOMATIC-{os.getpid()}] No source adapter directory found in candidates: {adapter_base_candidates}")

                copied_candidate_path = os.path.join(adapter_base_dir, adapter_filename)
                if os.path.exists(copied_candidate_path):
                    adapter_full_path = copied_candidate_path

            # 어댑터 파일 존재 확인
            if adapter_full_path is None or not os.path.exists(adapter_full_path):
                return _failure(
                    f"Adapter file not found: {adapter_filename}. Checked: "
                    + ", ".join(os.path.join(path, adapter_filename) for path in adapter_base_candidates)
                )
        ...
        if file_type == 'SE':
            ...
            if trimmomatic_params['trimmomatic_adapter_file']:
                cmd.append(
                    f"ILLUMINACLIP:{trimmomatic_params['trimmomatic_adapter_file']}:"
                    f"{trimmomatic_params['illuminaclip_seed_mismatches']}:"
                    f"{trimmomatic_params['illuminaclip_palindrome_clip']}:"
                    f"{trimmomatic_params['illuminaclip_simple_clip']}"
                )
