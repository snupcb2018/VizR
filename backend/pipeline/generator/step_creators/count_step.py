...
def create_count_step(job_definition: PipelineJobDefinition, step_name: str, step_counter: int) -> List[PipeLineStep]:
    ...
    if 'clean_tools' not in parameters:
        clean_step_config = next((s for s in pipeline_steps if s['step'] == 'clean'), None)
        if clean_step_config:
            parameters['clean_tools'] = clean_step_config.get('parameters', {}).get('clean_tools', [])
            logger.info(f"✅ [COUNT_STEP] Found clean_tools from pipeline config: {parameters['clean_tools']}")
        else:
            parameters['clean_tools'] = []
            logger.info("ℹ️ [COUNT_STEP] No clean step in pipeline config, using empty clean_tools")
